(function () {
  ...
  // FIX 7 (double-spinner): debounce the MutationObserver callback so that
  // large Dash DOM cascades (e.g. session load) trigger only ONE repaint pass
  // instead of hundreds of stacked requestAnimationFrame calls.
  // Also removed the mousedown double-setTimeout that was stacking repaints.
  var _paintTimer = null;
  function schedulePaint() {
    if (_paintTimer) return;            // already scheduled, skip
    _paintTimer = requestAnimationFrame(function () {
      _paintTimer = null;
      paint();
    });
  }
  ...
  function paint() {
    ...
    document.querySelectorAll(
      ".Select-value-label, .Select-value-label *, .Select-placeholder"
    ).forEach(function (el) {
      set(el, "color",                   TEXT);
      set(el, "-webkit-text-fill-color", TEXT);
      set(el, "opacity",                 "1");
    ...
    /* React-Select v5+ portal-rendered menus */
    ...
    document.querySelectorAll(
      'div[class*="-singleValue"], div[class*="-placeholder"], div[class*="-Input"] input'
    ).forEach(function (el) {
      set(el, "color",                   TEXT);
      set(el, "-webkit-text-fill-color", TEXT);
    });
  ...
  function start() {
    paint();
    // FIX 7: use schedulePaint (debounced via rAF) instead of calling
    // requestAnimationFrame directly — prevents stacked paint calls during
    // large Dash DOM updates which was causing duplicate spinner renders.
    ...
    // Removed the mousedown double-setTimeout — it stacked 2 extra paint
    // passes on every click and contributed to the double-spinner.
  }
